...
def _init():
    ...
    # ── USER STORE (persistent across reruns via session_state) ───────────────
    ...
    # ── PENDING APPROVALS (role-upgrade requests from non-main admins) ────────
    if 'pending' not in st.session_state:
        st.session_state.pending = []   # list of {req_by, target_user, action, payload}
    if 'refresh_counter' not in st.session_state:
        st.session_state.refresh_counter = 0
    if 'ms365_enabled' not in st.session_state:
        # Auto-detect: if all 3 file IDs are in secrets, enable live mode
        try:
...
.mode-tabs{{display:flex;gap:3px;background:#0c1018;border:1px solid rgba(255,255,255,.07);
  border-radius:10px;padding:3px;width:fit-content;margin:24px auto 0}}
.mtab{{padding:7px 22px;border-radius:7px;border:none;cursor:pointer;
  font-family:'DM Sans',sans-serif;font-size:12px;font-weight:600;
  color:#4a5068;background:transparent;transition:all .18s;white-space:nowrap}}
.mtab.on{{background:#4fce8f;color:#060910}}
.mtab.off{{color:#4a5068}}
...
.ibox{{background:rgba(79,142,247,.05);border:1px solid rgba(79,142,247,.12);
  border-radius:14px;padding:16px 20px;margin:20px auto;max-width:820px;
...
<div class="hh">
  ...
  <div class="hsub">{'Live Microsoft 365 data' if ms_on else 'Upload your Excel files'} · Generate 3 interactive dashboards</div>
</div>
<div class="dprow">
...
""", unsafe_allow_html=True)
  ...
  <strong>Live mode active:</strong> Dashboards pull directly from your Microsoft 365 Excel files.<br>
  Files are fetched fresh each time you click <strong>🔄 Refresh & Build</strong>.<br>
  Multiple team members can update the Excel files — just click Refresh to see latest data.
</div>""", unsafe_allow_html=True)
        ...
        with cb:
            if st.button("🔄  Refresh & Build Dashboards", use_container_width=True, type="primary", key="live_refresh"):
                with st.spinner("Fetching latest files from Microsoft 365…"):
                    try:
                        # Increment counter to bust st.cache_data
                        ...
                        s = data['stats']
                        st.success(
                            f"✅ Dashboards updated from live data — "
            ...
            # Config helper for admins
            if st.session_state.role == 'admin':
                with st.expander("⚙️ Microsoft 365 Configuration Help", expanded=False):
                    _show_ms365_setup()

    # ── MANUAL UPLOAD MODE ────────────────────────────────────────────────────
    else:
        st.markdown("""<div class="ibox">
  <strong>Manual upload mode:</strong> Upload your 3 Excel files below.<br>
  🔵 <strong>Free_Class_Lead_Report.xlsx</strong> — BCMB & INSIGNIA data (<code>BCMB</code> + <code>INSG</code> sheets)<br>
  🟠 <strong>Offline_Seminar_Report.xlsx</strong> — Seminar financials (<code>Offline Report</code> sheet)<br>
  🟣 <strong>Offline_Indepth_Details.xlsx</strong> — Students, payments, sales reps (multi-sheet)
</div>""", unsafe_allow_html=True)
        ...
        with cb:
            if wf and sf and af:
                ...
                st.markdown(f'''<div style="text-align:center;padding:14px;background:rgba(255,255,255,.02);
                  ...
                  Waiting for: <strong style="color:#8a90aa">{" · ".join(missing)}</strong></div>''',
                    unsafe_allow_html=True)
...
def show_dashboard():
    ...
    if not st.session_state.dashboards:
        st.markdown("<div style='padding:40px;text-align:center;color:#4a5068'>No dashboards yet. Upload files on Home page.</div>", unsafe_allow_html=True)
        _,cb,_ = st.columns([1,2,1])
        with cb:
...
<div class="aw">
  <div class="aw-hdr">
    ...
    <div>
      ...
      <div class="aw-sub">{'Main Admin — full control' if is_main else 'Admin — changes to critical actions require main admin approval'}</div>
    </div>
  </div>
</div>
""", unsafe_allow_html=True)
    ...
    # ── PENDING APPROVALS (main admin only) ────────────────────────────────────
...
<div class="ut-grid ut-hd">
  ...
  <div></div>
        ...
        # Action buttons (4 columns under "Actions")
  ...
  <span style="font-size:10px;color:#4a5068;font-family:DM Sans,sans-serif">Share with user. Use the Reset section below to apply new password.</span>
</div>''', unsafe_allow_html=True)
    ...
    # ── CHANGE PASSWORD (main admin only direct; others queue) ────────────────
    ...
    st.markdown('</div>', unsafe_allow_html=True)   # close .aw

# ─── ACTION HELPERS ────────────────────────────────────────────────────────────
def _queue(action, target, payload, req_by):
    """Queue a request for main admin approval."""
    st.session_state.pending.append({
        'action': action, 'target': target,
        'payload': payload, 'req_by': req_by
    })
    st.info(f"📨 Request queued — main admin approval required.")

def _apply_action(req):
    """Execute an approved action on the users dict."""
    users  = st.session_state.users
    target = req['target']
    action = req['action']
    if action == 'suspend':
